...
function createPersianTurbineUnit(wtdefinition) {
	// this function creates a full wind energy unit, including foundation
	// the input wtdefinition defines the turbine
	/////////////////////////////////////////////////////////////////////////////////alert("test 2" + wtdefinition)
	...
	// create foundation
	 ...
	 var foundation = createPersianFoundationObj([0,10,4,2,"brown"]);
	 WTU.add(foundation);
	//
	// // // create tower
	//  var tower = createTowerObj(wtdefinition.tower);
	...
	// tower.position.add(tempdeltapos1);
	// create nacelle
	var nacelle = createPersianNacelleObj([20,30,"yellow"]);
	nacellerotor.add(nacelle);

	// create rotor, including hub
 	var rotor = createPersianRotorObj([20,30,"red","white"]); // define the object as 3D

 // 	WTU.add(rotor);
//
// 	rotorshaft.add(rotor);
// 	rotorshaft.rotation.z = wtdefinition.nacelle[6];
 	nacellerotor.add(rotor);
// //	nacellerotor.position.set(0,wtdefinition.nacelle[1]*wtdefinition.nacelle[2]/2,0);
//
 	nacellerotor.position.set(0,10,0);
...
function createPersianFoundationObj(definition) {
	...
	var geom1 = new THREE.BoxGeometry(definition[2],1,definition[3],1,1,1); // define geometry
...
function createPersianRotorObj(definition) {
	// this function creates a  rotor
	// the input definition defines the foundation
	//////////////////////////////////////////////////////////////////////////////////////////////////alert("test 3" + definition)
	var found = new THREE.Object3D(); // define the object as 3D	found.name = "foundation";
	found.name = "rotor";
	// create hub
	var hub = createPersianHubObj(definition[2]);
	...
	//place blades at the end of the hub (almost at end)
	var tempdeltapositionblades = new THREE.Vector3( 0., 0.5, 0 );
	...
	for (i = 0; i < nblades; i++) {
		bladeind= i +1 ;
		blade = createBladeObj(["Persian1", definition[2], definition[3]], bladeind);
		blade.position.add(tempdeltapositionblades);
		blade.rotation.y=deltaangle*i;
		found.add(blade) ;
	};
	// add rotor elements
	found.add(hub) ;
  found.scale.set(definition[0],definition[1],definition[0])
...
function windenergyunit(inputweu){
    ...
    this.updateWEU= function(deltat){
      ...
      this.update_cost_om(deltat)
		...
		// method to calculate revenue
    this.updateWEU_geometry= function(wtunit, deltat){
			var trans1;
			var winddirection;
			winddirection = Math.atan2(wind.z,-wind.x);
			var rotorpointer = wtunit.getObjectByName( "rotor" );
			var nacellerotorpointer = wtunit.getObjectByName("nacelle");
			// trans1 = 0.2*delTime;
			// nacellerotorpointer.rotation.y -= trans1;
			nacellerotorpointer.rotation.y = winddirection;
			trans1 = 0.002*deltat;
			rotorpointer.rotation.y += trans1;
    };
...
//		element.scale.set(10,10,10)
	...
	if(definition[0]=="Persian1"){
		var geom1 = new THREE.BoxGeometry(0.2,1,0.01,1,1,1); // define geometry

		var mat1 = new THREE.MeshPhongMaterial({color:Colors[definition[1]], shading:THREE.FlatShading}); // define material
		var element = new THREE.Mesh(geom1, mat1); // create element
		element.castShadow = true; // define properties of the elements
		element.receiveShadow = true;
	//	element.scale.set(10,10,10)
		element.position.set(0.1+0.25,0.1,0);
		found.add(element);  ///

		var geom1 = new THREE.BoxGeometry(0.5,1,0.005,1,1,1); // define geometry

		var mat1 = new THREE.MeshPhongMaterial({color:Colors[definition[2]], shading:THREE.FlatShading}); // define material
		var element = new THREE.Mesh(geom1, mat1); // create element
		element.castShadow = true; // define properties of the elements
		element.receiveShadow = true;
//		element.scale.set(10,10,10)
...
function createPersianHubObj(definition) {
	// this function creates a  Hub
	// the input definition defines the Object
	//////////////////////////////////////////////////////////////////////////////////////////////////alert("test 3" + definition)
	var found = new THREE.Object3D(); // define the object as having a 3D mesh
	found.name = "hub";
	var geom1 = new THREE.CylinderGeometry(0.075,0.075,1,5); // define geometry
	//	var geom1 = new THREE.CylinderGeometry(definition[4],definition[5],definition[6],10); // define geometry
	var mat1 = new THREE.MeshPhongMaterial({color:Colors[definition[0]], shading:THREE.FlatShading}); // define material
	var element = new THREE.Mesh(geom1, mat1); // create element
	element.castShadow = true; // define properties of the elements
	element.receiveShadow = true;
	// element.rotation.z = Math.PI/2;
...
function createPersianNacelleObj(definition) {
	// this function creates a  foundation
	// the input foundationdefinition defines the foundation
	//////////////////////////////////////////////////////////////////////////////////////////////////alert("test 3" + definition)
	var found = new THREE.Object3D(); // define the object as having a 3D mesh
	...
	var geom1 = new THREE.BoxGeometry(0.05,1,1,1,1,1); // define geometry
	//	var geom1 = new THREE.CylinderGeometry(definition[4],definition[5],definition[6],10); // define geometry
	var mat1 = new THREE.MeshPhongMaterial({color:Colors[definition[2]], shading:THREE.FlatShading}); // define material
	var element = new THREE.Mesh(geom1, mat1); // create element
	element.castShadow = true; // define properties of the elements
	element.receiveShadow = true;
	element.position.set(-0.7,0.5,0)

	found.scale.set(definition[0],definition[1],definition[0])

	found.add(element);  ///

	var geom1 = new THREE.BoxGeometry(0.05,1,1,1,1,5); // define geometry
	//	var geom1 = new THREE.CylinderGeometry(definition[4],definition[5],definition[6],10); // define geometry
	var mat1 = new THREE.MeshPhongMaterial({color:Colors[definition[2]], shading:THREE.FlatShading}); // define material
	var element = new THREE.Mesh(geom1, mat1); // create element
	element.castShadow = true; // define properties of the elements
	...
	for (var i = 0; i < geom1.vertices.length; i++) {
		var vertex = geom1.vertices[i];
		 vertex.x += -(1-Math.cos(Math.asin(Math.min(vertex.z,0)/0.7)));
	};
